    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            if (! (`${x},${y}` in freeCells)) { // only want to place a wall somewhere if it's not a free cell
                ...
                let sym = sums[sum]+1;
                map.layers[0].data[y][x] = sums[sum]+1;
            }
        }
    }
...
export function randomDungeon(width, height, dir, level = 1) {
    ...
    for (let j = 0; j < height; j++) {
        ...
        for (let i = 0; i < width; i++) {
            if (i + "," + j in freeCells)
                map.layers[0].data[j].push(7858) // center stone wall floor
            else
                map.layers[0].data[j].push(7046) // black tiles
    ...
    let buildsCorrWalls = function(x, y, horizontal, beginning = false, end = false) {
        let floor_ids = Object.values(corridorFloor.vertical) + Object.values(floor) + Object.values(corridorFloor.horizontal) + [7741, 569, 568] // doors?
        let corner_ids = [walls.upperLeft, walls.upperRight, walls.lowerLeft, walls.lowerRight]
        if (horizontal) {
        ...
        if (end || beginning) {
            /* TODO : check and see if the end tile connects to
            another corridor and place a corner floor tile */
            ...
            /* if we are laying a horizontal corridor, then we also need to resolve
               ...
               if the cases are SE or NE, then we know that
               the conflicting endpoint with some vert corr
               is going to be the right most tile of the horiz corr
            ...
            if (horizontal && end) {
                ...
                */
                if (freeCells[x + ',' + y] &&
                    ...
                    freeCells[(x + 1) + ',' + y] &&
                    freeCells[(x + 2) + ',' + y]) {
                        console.log(`placing lower left wall corner at  ${x + 1}, ${y - 1}, case 1`)
                        map.layers[0].data[y - 1][x + 1] = walls.lowerLeft + 1

                } else if (freeCells[x + ',' +y] &&
                            freeCells[x-1 + ',' + y] && freeCells[x+1 + ',' + y] && freeCells[x-1 + ',' + (y-1)] && freeCells[x-1 + ',' + (y-2)]) {
                                console.log(`placing lower left wall corner at  ${x}, ${y - 1}, case 2`)
                ...
                */
                if (freeCells[x+','+y] && freeCells[x+1 + ',' + y] && freeCells[x+2+','+y] && freeCells[x + ',' + (y+1)] && freeCells[x + ',' + (y+2)]) {
                    console.log(`placing upper left wall corner at  ${x + 1}, ${y + 1}`)
                    ...
                    console.log(`placing upper left wall corner at  ${x}, ${y + 1}`)
    ...
    for (let corridor of rogueMap.getCorridors().sort(sortCorridors)) {
        ...
        if (sx === ex) { // vertical corridor |
            ...
            let y = sy
            buildsCorrWalls(sx, y, false, true, false)
            map.layers[0].data[y][sx] = corridorFloor.vertical.top + 1
            y++
            while (y < ey) {
            ...
            map.layers[0].data[ey][sx] = corridorFloor.vertical.bottom + 1
            buildsCorrWalls(sx, y, false, false, true)
        } else if (sy === ey) { // horizontal corridor ------
            if (sx > ex) {
                let temp = sx
                sx = ex
            ...
            let x = sx
            buildsCorrWalls(x, sy, true, true, false)
            map.layers[0].data[sy][x] = corridorFloor.horizontal.left + 1
            x++
            while (x < ex) {
            ...
            buildsCorrWalls(x, sy, true, false, true)
            map.layers[0].data[ey][x] = corridorFloor.horizontal.right + 1
        } else {
            console.log(`[${sx}, ${sy}] => [${ex},${ey}]`)
        }
